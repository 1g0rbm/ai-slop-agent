...
умолчанию модель получает последние 20 завершённых пар запрос-ответ без
изменений. Лимит можно изменить переменной `HISTORY_LIMIT`:

```sh
export HISTORY_LIMIT=20
```

Более старые пары заменяются в контексте накопительным кратким содержанием
(summary). Полная история при этом не удаляется из SQLite. Summary хранится
отдельно вместе с checkpoint последней обработанной пары и подставляется после
системного промпта перед свежей историей.

По умолчанию summary обновляется отдельным вызовом выбранной модели после того,
как за пределами свежего контекста накопятся 10 новых сообщений, то есть 5
полных пар. Размер блока можно изменить:

```sh
export SUMMARY_BATCH_MESSAGES=10
```

`SUMMARY_BATCH_MESSAGES` должен быть положительным чётным числом: история
суммаризируется только полными парами `user`/`assistant`. Каждый такой блок
вызывает дополнительный запрос к провайдеру. Новое summary строится из
предыдущего summary и очередного блока сообщений, поэтому вся старая история
повторно не отправляется.

Значение `HISTORY_LIMIT=0` исключает из основного контекста предыдущие пары и
постепенно переносит их в summary. Значение `-1` передаёт всю ещё не
суммаризированную историю и отключает дальнейшее сворачивание. Уже созданное
summary продолжает использоваться, поскольку разворачивать его обратно в
исходные сообщения небезопасно. Текущий запрос и системный промпт передаются
всегда.
...
Команда `/clear` удаляет историю и сохранённое summary после подтверждения. Для
завершения сеанса используйте `exit`, `quit`, Ctrl-D или Ctrl-C.
